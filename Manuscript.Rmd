...
``` {r ROI computation from Matlab, echo=FALSE, message = FALSE}
# Based on Yeo et al. 17 network parcellation
# These only include areas in the medial wall belonging to DMN + OFC (changed on 12/4/18)
Yeo_labels <- c("L_25_",
                "L_OFC_",
                "L_10v_",
                "L_s32_",
...
# This version is literally what I had selected originally, plus 25, OFC, 10v. This replicates the 7m finding. 
...
# Perform the prop test for each pair and get the max chi-squared stat
tmpDF$Chi_squared <- apply(tmpDF[,2:3], 1, function(data) {round(prop.test(data, c(length(studyList_sv), length(studyList_dmn)))$statistic, digits = 2)})

plot(density(nullDist))

quantile(nullDist, 0.95)

tmpDF %>% filter(Chi_squared > quantile(nullDist,0.95))
```
...
test <- do.call(rbind, Summaries)
test %>%
  mutate(SubjID = rep(subjList, each = 5081),
         FV_abs = abs(FiedlerVec),
         Community = ifelse(FiedlerBinary == 0, "nonDMN", "DMN")) %>%
  filter(y > 0) %>%
  group_by(SubjID, Community) %>%
  summarize(FV_mean = median(FV_abs)) %>%
  ggplot(aes(Community, FV_mean)) +
  geom_boxplot() + 
  theme_classic()
# Second one plots the ecdf
# Line indicates the 75th quantile of the combined Fiedler vectors across participants (i.e. potential threshold)
test %>%
  mutate(SubjID = rep(subjList, each = nrow(Summaries[[1]])),
         FV_abs = abs(FiedlerVec),
         Community = ifelse(FiedlerBinary == 0, "nonDMN", "DMN")) %>%
  filter(y > 0) %>%
...
# Load the correlation-based maps (this might be ok to move to the cor vs comm chunk)
...
Make sure to emphasize that the high overlap between sliding windows makes it more likely that differences are due to noise in the signal, and not strong variability. Like, you wouldn't expect the actual network distribution to shift that much after shifting the time series by 1 minute.g
