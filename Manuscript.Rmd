...
Halves_mPFCvPCC__Modularity_permTest <- permute(Halves_RIRatios_PFC_Modularity, Halves_RIRatios_PCC_Modularity, paired = T, statType = median)
...
curvCors %>% group_by(region, Hemisphere) %>% 
  summarise(TStat = t.test(Correlation)$statistic,
...
```{r Extra examples (supplemental fig 1), out.width = "100%", fig.align = "center", fig.cap = "Additional examples of individualized partitionings, showing both Fiedler vector values (left) and binarized communities (right). A common organizational principle is visible, even though it shifts topographically across individuals."}
