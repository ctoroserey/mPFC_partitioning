...
#          FiedlerVec > 0) %>% 
#   group_by(SubjID, region, Curvature) %>%
#   summarise(mFV = mean(FiedlerVec)) %>%
#   group_by(region, Curvature) %>%
#   summarise(meanFV = mean(mFV),
#             seFV = sd(mFV, na.rm = T) / sqrt(100)) %>%
#   ggplot(aes(Curvature, meanFV, color = region)) +
#     geom_line(size = 1.5) +
#     labs(y = "Mean Fiedler Vector Value", color = "Region") +
#     geom_errorbar(aes(Curvature, ymin = meanFV - seFV, ymax = meanFV + seFV), width = 0.05, size = 1.5) +
...
t <- do.call(rbind, Summaries) %>%
  mutate(Curvature = round(Curvature, digits = 1),
         region = factor(region, levels = list("vmPFC", "residual_mPFC", "PCC"))) #%>%
  #filter(abs(FiedlerVec) > 0.01)

glmodel2 <- glmer(FiedlerBinary ~ Curvature * region + (1 | SubjID), data = t, family = "binomial")
