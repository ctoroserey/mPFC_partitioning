...
``` {r ROI computation, echo=FALSE, message = FALSE}
  ...
  # This part is just for reference, as the labels are loaded from a file
...
``` {r Partition comparison: halves plot, fig.align="center", fig.width = 6, fig.height = 4, fig.cap = "Figure 5. Similarity matrices showing pattern agreement across days for the whole partition (left), and for PCC and mPFC separately (right). These figures show high agreement within (but not between) subjects, with mPFC again showing significant variability.", echo=FALSE, message = FALSE, cache = TRUE}
# plots
par(mfrow=c(1,2))
...
``` {r Partition comparison: session similarity matrices plot, fig.align="center", fig.width = 6, fig.height = 4, fig.cap = "Figure 7. Similarity matrices showing pattern agreement across sessions for the whole partition (left), and for PCC and mPFC separately (right). The pattern from the daily analysis is preserved, with marked individuality for mPFC, but not PCC.", echo=FALSE, message = FALSE, cache = TRUE}
# plot
par(mfrow=c(1,2))
...
``` {r Stability per subject: Overall, fig.align="center", fig.width=6, fig.height=4, fig.cap = "Figure 9. ARI between communities computed at each sliding epoch and the original partition for all subjects (left), as well as the distribution of ARI standard deviations per subject (right).", echo=FALSE, message = FALSE, cache = TRUE}
# Load files
...
``` {r Stability per subject: PCC vs mPFC, fig.align="center", fig.width=10, fig.height=4, fig.cap = "Figure 11. ARI between each window and the original pattern for PCC and mPFC separately (left), and the mean per-subject ARI for the whole group (middle). Regardless of the mean ARI, standard deviation remained low for both regions (right).", echo=FALSE, message = FALSE, cache = TRUE}
...
This suggests that, while the agreement of each window with the overall time series was stable for both PCC and mPFC (low SD, right plot), the overall agreement of mPFC is lower (paired permutation, p = `r SW_PCCvPFC_permuteTest$Pval`; Cohen's D = `r round(SW_PCCvPFC_ES, digits = 2)`). 

\bigskip

### *Potential Noise Effects*

This result could be due to signal degeneration in mPFC (that said, it is worth noting that the topological patterns remain mostly the same for both PCC and mPFC throughout the SW analysis). To account for this, we computed the temporal signal to noise ratio (tSNR) for each vertex in each subject's minimally preprocessed fMRI data (i.e. mean of each vertex's time points over their standard deviation). The next plot shows the pooled subject-wise mean tSNR for PCC and mPFC vertices, and indicates that the tSNR is lower for mPFC than PCC, which could negatively affect our results.


``` {r Noise Effects: tSNR Comparison, fig.align="center", fig.width=3.5, fig.height=3.5, fig.cap = "Figure 12. Mean tSNR for mPFC and PCC across participants. This measure was derived from the minimally preprocessed fMRI data from the HCP, and thus shows the base level of reltive noise in each region.", echo=FALSE, message = FALSE, cache = TRUE}
...
``` {r Noise Effects: Per-subject Fiedler by R-sqd, fig.align="center", fig.width=4, fig.height=4, fig.cap = "Figure 13. Fiedler vector values are linearly related to the total variance explained by each vertex with respect to the mean DMN time series.", echo=FALSE, message = FALSE, cache = TRUE}
setwd('./Variance/')
temp <- list.files()
...
``` {r Noise Effects: Mean R-sqd mPFC and PCC, fig.align="center", fig.width=6, fig.height=4, fig.cap = "Figure 14. Differences in mean R-squared values for each region per participant. The figure shows that PCC vertices tend to be more coupled with the overall dynamics of DMN across subjects.", echo=FALSE, message = FALSE}
v4
```
...
``` {r Noise Effects: R-sqd vs tSNR, fig.align="center", fig.width=6, fig.height=4, fig.cap = "Figure 15. Vertex tSNR values across all subjects are correlated with their position along the z-axis in mPFC, with vmPFC showing the least tSNR (left). However, R-square values do not show this strong relationship (right), suggesting that the coupling of each vertex is not necessarily influenced by signal noise.", echo=FALSE, message = FALSE, cache = TRUE}
...
tempVariance <- allVariance[allVariance$Region == "mPFC", ]
s1 <- ggplot(data = allVariance, aes(Rsquared, tempZ)) +
  geom_bin2d(bins = 250) +
...
s2 <- ggplot(data = allVariance, aes(varCoeff, tempZ)) +
  geom_bin2d(bins = 250) +
  scale_fill_continuous(low = "grey30", high = "grey70") +
  geom_density2d(color = Cols[2]) +
...
``` {r Correlation vs community detection: plot, fig.align="center", fig.width = 4, fig.height = 4, fig.cap = "Figure 16. Correlation matrix comparing the inter-day spatial stability of maps derived from seed-based functional connectivity (7m-based) and the Fiedler vector. The top-left quadrant represents seed-based FC maps, and the bottom-right the Fiedler vector.", echo=FALSE, message = FALSE, cache = TRUE}
...
Add a portion showing that summing up the binarized maps produces a map similar to the one produced by Margulies et al. and others.
...
### *Table 1. Significant brain regions per literature*
