...
``` {r ROI computation (overlapping: old), echo=FALSE, message = FALSE}
...
snrR2_cors$pre_thresh <- sapply(appendVar, function(data) {
  data %>% 
    filter(!is.na(Rsquared)) %>%
    select(varCoeff, Rsquared) %>%
    cor(.$varCoeff)
})[2, ]

snrR2_cors$post_thresh <- sapply(appendVar, function(data) {
  data %>% 
...
# next get the mean tSNR of PCC and mPFC before/after
snrR2_means <- list()
snrR2_means <- sapply(appendVar, function(data) {
  data %>% 
    filter(!is.na(Rsquared)) %>%
    mutate(region = ifelse(z > 0, "mPFC", "PCC")) %>%
    group_by(region) %>% 
    summarize(mean_tsnr = mean(varCoeff)) %>%
    ungroup()
})
