...
curvCors %>% group_by(region, Hemisphere) %>% 
  summarise(medianCor = median(Correlation),
            seCor = sd(Correlation) / sqrt(nSubj),
            TStat = t.test(Correlation)$statistic,
            p = t.test(Correlation)$p.val,
            Sig = p < 0.05,
...
```{r Association with sulci (Fig. 8), fig.align = "center", fig.cap = "A: Correspondence between SP communities and sulcal morphology in two example individuals. Zoomed-in boxes show areas in PCC and ventral mPFC where the DN community appears to be guided by sulcal organization. This relationship is less defined in superior mPFC. The superior rostral sulcus houses the DN community, in agreement with findings by Lopez-Persem et al. (2019). B: We quantified this relationship by correlating cortical curvature with the unthresholded Fiedler values. We divided the search space into three areas (left): ventral mPFC (yellow), superior mPFC (sup-mPFC, red), and PCC (blue). Correlations on each combination of region and hemisphere per individual showed that DN (signaled by positive FV values) tends to be represented in vmPFC sulci, but is likely located in gyri within sup-dmPFC. PCC has an inversed relationship across hemispheres, with DN landing more often in left hemisphere sulci."}
# # box plot
...
Next, we asked if the idiosyncratic organization of the DN was reflected by patterns of sulcal morphology. A variety of studies have provided evidence that suclal and gyral organization can inform the location of functional effects (*PAPERS*). Among these, recent work has suggested that the location of the DN in individuals lies mostly within sulci in vmPFC [@Lopez2019]. We sought to replicate this relationship using the SP communities, as well as interrogating whether it persisted in PCC and more dorsal mPFC regions.

Here you'll want to mention a number of things:

- in vmpfc ROS-S contains DN, just like L-P
- hemispheric differences in PCC
- the papers given by rev 3
