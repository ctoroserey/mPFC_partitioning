...
We examined whether the location of the DN and non-DN communities was guided by sulcal morphology. 
...
primarySulci <- c("L_a24_ROI", "R_a24_ROI", "L_s32_ROI", "R_s32_ROI", "L_p32_ROI", "R_p32_ROI", "L_10r_ROI", "R_10r_ROI")

indx <- getCoords(Labels = persemROI, Coords = Summaries[[1]])$Index
vmpfc <- rep(0, nVertices)
vmpfc[indx] <- 1

# update Summaries dfs, and reduce to vmPFC only
curvData <- lapply(files, function(file) {read.table(file, header = F)$V1[Summaries[[1]]$Vertex]})
for (i in seq(nSubj)) {
  Summaries[[i]]$Curvature <- curvData[[i]]
  Summaries[[i]]$SubjID <- subjList[[i]]
  Summaries[[i]]$vmpfc <- vmpfc
  Summaries[[i]] <- Summaries[[i]] %>% mutate(region = case_when(
    y < 0 ~ "PCC",
    vmpfc == 1 ~ "vmPFC",
    TRUE ~ "dmPFC"
  ),
...
# # mimic the binning and averaging L-P did on vmPFC
# # this will plot a similar figure for a similar mask, plus a more dorsal mPFC one and PCC
# do.call(rbind, Summaries) %>%
#   mutate(Curvature = round(Curvature, digits = 1)) %>%
#   filter(Curvature > -0.5) %>%
#   group_by(SubjID, region, FiedlerBinary, Curvature) %>%
#   summarise(mFV = mean(FiedlerVec)) %>%
#   group_by(region, FiedlerBinary, Curvature) %>%
#   summarise(meanFV = mean(mFV),
#             seFV = sd(mFV, na.rm = T) / sqrt(100)) %>%
#   ggplot(aes(Curvature, meanFV, color = region, linetype = as.character(FiedlerBinary))) +
#     geom_line(size = 1.2, show.legend = F) +
#     labs(y = "Mean Fiedler Vector Value", color = "Region") +
#     geom_errorbar(aes(Curvature, ymin = meanFV - seFV, ymax = meanFV + seFV), width = 0.08, size = 1.2, linetype = "solid", show.legend = F) +
#     scale_x_continuous(breaks = c(-0.4, 0, 0.4, 0.8)) +
#     geom_point(size = 2) +
#     scale_color_manual(values = c("firebrick", "dodgerblue4", "gold2")) +
...
# MIXED EFFECTS LOGISTIC ESTIMATING SULCUS FROM FV AND REGION, WORKS BUT CHANGE REFERENCE
sulcData <- do.call(rbind, Summaries) %>%
  mutate(Curvature = round(Curvature, digits = 1),
         region = factor(region, levels = list("vmPFC", "dmPFC", "PCC"))) #%>%
  #filter(abs(FiedlerVec) > 0.01)

sulcGLM <- list()
sulcGLM$vmpfc <- glmer(FiedlerBinary ~ Curvature * region + (1 | SubjID), data = sulcData, family = "binomial")
sulcData$region <- factor(sulcData$region, levels = list("dmPFC", "vmPFC", "PCC"))
sulcGLM$dmpfc <- glmer(FiedlerBinary ~ Curvature * region + (1 | SubjID), data = sulcData, family = "binomial")

# right now it looks like vmpfc FV is negatively related to curvature, but the opposite is true for dmPFC
#lapply(sulcGLM, summary)
