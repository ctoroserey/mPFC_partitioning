...
Figure 6 shows these ARI ratios for PCC and mPFC for all individuals As expected, we found that while both regions showed evidence for specificity (i.e. ratios > 1), the ratios for mPFC (median = `r round(median(Halves_RIRatios_PFC), digits = 2)`; IQR = `r quantile(Halves_RIRatios_PFC, 0.25)` - `r quantile(Halves_RIRatios_PFC, 0.75)`) were significantly higher than those for PCC (median = `r round(median(Halves_RIRatios_PCC), digits = 2)`; IQR = `r quantile(Halves_RIRatios_PCC, 0.25)` - `r quantile(Halves_RIRatios_PCC, 0.75)`) when compared with a paired permutation test (*p* `r ifelse(Halves_mPFCvPCC_permTest$Pval < 0.001, "< 0.001", paste("=", Halves_mPFCvPCC_permTest$Pval))`; Cohen's D = `r round(Halves_mPFCvPCC_ES, digits = 2)`) **(TBD: test against 1. I think it might be worth doing it for PCC's sake, as mPFC's numbers + figure already convey its idiosyncrasy very well. If we do it here, we'll do it for the run-specific analysis too)**. These test/retest results suggest that the topographic variability seen in mPFC arises at least in part from stable and subject-specific organizational patterns.
