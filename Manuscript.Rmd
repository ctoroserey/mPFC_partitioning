...
persemROI <- c("_25_", "_s32_", "_a24_", "_10v_", "_10r_", "_p32_")
...
curvVmpfc <- lapply(Summaries, getCoords, Labels = persemROI)
curvVmpfc <- lapply(curvVmpfc, "[[", "Coords")

# compute per-subject correlations between Fiedler Vector and curvature
curvCors <- sapply(curvVmpfc, function(subj) {cor(subj$Curvature, subj$FiedlerVec)})


# mimic the binning and averaging they did
do.call(rbind, curvVmpfc) %>%
  mutate(Curvature = round(Curvature, digits = 1)) %>%
  filter(Curvature > -0.3,
         FiedlerVec > 0.01) %>%
  group_by(SubjID, Curvature) %>%
  summarise(mFV = mean(FiedlerVec)) %>%
  group_by(Curvature) %>%
  summarise(meanFV = mean(mFV),
            seFV = sd(mFV, na.rm = T) / sqrt(100)) %>%
  ggplot(aes(Curvature, meanFV)) +
    geom_point(show.legend = F, size = 1.5) +
    geom_errorbar(aes(Curvature, ymin = meanFV - seFV, ymax = meanFV + seFV), width = 0.01, size = 1) +
    theme_classic()
```
