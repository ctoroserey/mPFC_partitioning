...
ecdfPCC <- test %>%
          mutate(SubjID = rep(subjList, each = nVertices),
...
#grid.arrange(All,pcc2,pfc2, ncol = 1)
...
for (i in seq_along(tempList)) {
  if (is.null(tempList[[i]]$result)) {
    ...
    threshDF[i,2] <- tempList[[i]]$result[1]
    threshDF[i,3] <- tempList[[i]]$result[2]
  }
}
```
