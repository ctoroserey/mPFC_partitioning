...
<!--
We also diagnosed our choice of two communities by reporting the rank (from lowest to largest) of the first eigenvalue produced by SP to be non-zero. This eigenvalue serves as an estimator of the optimal number of communities in a network (Chung 1999), such that a rank of two in our partitions justifies limiting the number of communities to two.
-->
...
Our results are complementary with previous studies that described individual-specific brain organization. On one hand, topographical heterogeneity has been qulitatively highlighted using a different data aspect ratio [few individuals and more data per individual; @Gordon2017, @Braga2017, @Braga2019]. In terms of stability, previous work has relied on parcellations and group priors to show that an individual's functional connectivity pattern is consistent in time [@Gratton2018; @Kong2018], thus being relatively insensitive to an individual's complex topography. Here we were able to quantify the variability and stability of fine-scale functional topography in a large sample with moderately low amounts of data (down to a single 15 minute scan). The motivation to sub-divide DN also stems from recent work by Kernbach et al. [-@Kernbach2018], where they identified specialized communication of parcels within DN with the rest of the brain in a large pool of individuals.
  
We found SP results could be made more stable by thresholding the underlying Fiedler vector. We generated custom thresholds for each individual by interrogating the relationship between Fiedler vector values and the degree of persistent community affiliation of vertices over time. The magnitude of these eigenvector values has been recently used to characterize the continuous connectivity profile of the insula with the rest of the brain, challenging the notion of discrete parcellations of this region [@Tian2018]. We anticipate that these values will be helpful to future neuroscientific studies in providing gradual information not afforded by other clustering algorithms.
...
And write a portion on the lack of subcortical reward-related regions (i.e. striatum). 
