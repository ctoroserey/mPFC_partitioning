...
Outline:
  ...
  - It matters whether distinct cognitive functions overlap in the brain. Apparent overlap has motivated theoretical proposals about ways in which seemingly dissimilar cognitive domains might recruit a shared set of core cognitive processes. 

  - The interpretation of overlap in group-level data depends on the degree to which functional organization is heterogeneous across individuals. Group-level and meta-analysis-level overlap does not necessarily imply overlap in individual brains. Individual-level effects wash out in group averages (Michalka), apparent overlap in functional activation can vanish with increases in spatial precision (Woo), and resting-state networks have reliable individual-specific characteristics (Muller, Braga). 

  - We don't yet really understand how much individual-level heterogeneity there is in the functional topography of mPFC. We need to (1) develop an efficient/robust method to capture individual-level topography, and (2) quantify the degree of heterogeneity in a large group of individuals. 

  - The method we propose is spectral partitioning. Restricting our analyses to a general mPFC/PCC search space makes it appropriate to use a technique that identifies a vertex-wise, binary partitioning. This contrasts with whole-brain network analyses, which need to allow for multiple sub-networks and which often use coarser-grained parcels as the units of analysis. SP has advantages (deterministic, simple diagnostics). Comparisons with modularity maximization suggested the binary partitioning was appropriate. 
