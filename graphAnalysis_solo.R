...
### Run a meta-analysis and graph theoretic fMRI analysis
...
if (Meta == T) {
  ...
  write("Setting up meta-analysis matrices...", stdout())
  
  # set directory for subjective value
  setwd('./DEC/')
  
  # number of studies and vector of study names
  nStudies <- length(dir()) - 1
  stuDir <- dir()[1:nStudies]
  
  # create empty matrix
  valMatrix <- matrix(data = 0, nrow = nrow(labelCoords_parcel), ncol = nrow(labelCoords_parcel))
  dimnames(valMatrix) <- list(labelCoords_parcel$Label, labelCoords_parcel$Label)
  
  # populate the matrix with 1 for every connection
  for (study in stuDir){
    
    # load a study
    tempData <- read.csv2(study, header = FALSE)
    
    # create an index for the corresponding row/columns (use rownames(valMatrix[indx,]) to check)
    indx <- numeric()
    for (parcel in tempData[[1]]){ indx <- c(indx, which(rownames(valMatrix) == parcel)) }
    
    # populate matrix for each combination
    valMatrix[indx,indx] <- valMatrix[indx,indx] + 1
    
  }
  
  # turn diagonals into 0
  diag(valMatrix) <- 0
  
  
  
  # set directory for DMN
  setwd('./DMN/')
  ...
  # create empty matrix
  dmnMatrix <- matrix(data = 0, nrow = nrow(labelCoords_parcel), ncol = nrow(labelCoords_parcel))
  dimnames(dmnMatrix) <- list(labelCoords_parcel$Label, labelCoords_parcel$Label)
  ...
  for (study in stuDir){
    ...
    # create an index for the corresponding row/columns (use rownames(valMatrix[indx,]) to check)
    indx <- numeric()
    for (parcel in tempData[[1]]){ indx <- c(indx, which(rownames(dmnMatrix) == parcel)) }
    
    # populate matrix for each combination
    dmnMatrix[indx,indx] <- dmnMatrix[indx,indx] + 1
...
###------- Meta-analysis: Compute ---------------
if (Meta == T) {
  
  stdout("Analyzing meta-analytic graphs...", stdout())
  
  # note: use get.data.frame(valGraph_s) to get a data frame with individual edges specified per row.
  
  # value
  # first level of thresholding. Basically "the least amount of co-occurrences in pairs that are worth considering" (best so far = 8)
  # Best thresholding method so far: I bootstrapped the edge weights for the POS condition (since DEC can be thought of as a theoretical subset of POS, giving me more samples), and took the   95th quantile (one-sided) from the resulting distribution. The final threshold is ~8. This way I don't have to also threshold based on vertex degree.
  thresh1 <- 8 
  valMatrix[valMatrix < thresh1] <- 0
  
  # create graph and assign V/E attributes
  valGraph <- graph.adjacency(valMatrix, mode = "undirected", weighted = TRUE)
  V(valGraph)[1:180]$hemi <- "Left"
  V(valGraph)[181:360]$hemi <- "Right"
  V(valGraph)[hemi == "Left"]$color <- Cols[1]
  V(valGraph)[hemi == "Right"]$color <- Cols[2]
  V(valGraph)$size <- sqrt(graph.strength(valGraph))
  V(valGraph)$size <- V(valGraph)$size * .5
  V(valGraph)$label <- gsub("_ROI","",V(valGraph)$name)
  V(valGraph)$label <- gsub("L_","",V(valGraph)$label)
  V(valGraph)$label <- gsub("R_","",V(valGraph)$label)
  #V(valGraph)$label <- V(valGraph)$name
  E(valGraph)$width <- E(valGraph)$weight / 3
  Left <- V(valGraph)[hemi == "Left"]
  Right <- V(valGraph)[hemi == "Right"]
  E(valGraph)[ Right %--% Right]$color <- Cols[2]
  E(valGraph)[ Left %--% Right]$color <- Cols[3]
  E(valGraph)[ Left %--% Left]$color <- Cols[1]
  
  # to remove poorly and unconnected vertices
  # bootGraph <- bootstrap(group = degree(valGraph), statType = median)
  # thresh2 <- quantile(bootGraph, 0.95)
  valGraph_s <- delete.vertices(valGraph, degree(valGraph)==0) #delete.vertices(valGraph, degree(valGraph) < thresh2)
  ...
  # Best thresholding method so far: I bootstrapped the edge weights for the POS condition (since DEC can be thought of as a theoretical subset of POS, giving me more samples), and took the 95th quantile (one-sided) from the resulting distribution. The final threshold is ~8. This way I don't have to also threshold based on vertex degree.
  thresh1 <- 10 
  dmnMatrix[dmnMatrix < thresh1] <- 0
  
  # create graph and assign V/E attributes
  dmnGraph <- graph.adjacency(dmnMatrix, mode = "undirected", weighted = TRUE)
  V(dmnGraph)[1:180]$hemi <- "Left"
  V(dmnGraph)[181:360]$hemi <- "Right"
  V(dmnGraph)[hemi == "Left"]$color <- Cols[1]
  V(dmnGraph)[hemi == "Right"]$color <- Cols[2]
  V(dmnGraph)$size <- sqrt(graph.strength(dmnGraph))
  V(dmnGraph)$size <- V(dmnGraph)$size * 0.2
  V(dmnGraph)$label <- gsub("_ROI","",V(dmnGraph)$name)
  V(dmnGraph)$label <- gsub("L_","",V(dmnGraph)$label)
  V(dmnGraph)$label <- gsub("R_","",V(dmnGraph)$label)
  #V(dmnGraph)$label <- V(dmnGraph)$name
  #E(dmnGraph)$width <- E(dmnGraph)$weight / 3
  Left <- V(dmnGraph)[hemi == "Left"]
  Right <- V(dmnGraph)[hemi == "Right"]
  E(dmnGraph)[ Right %--% Right]$color <- Cols[2]
  E(dmnGraph)[ Left %--% Right]$color <- Cols[3]
  E(dmnGraph)[ Left %--% Left]$color <- Cols[1]
  
  # to remove poorly and unconnected vertices
  # bootGraph <- bootstrap(group = degree(valGraph), statType = median)
  # thresh2 <- quantile(bootGraph, 0.95)
  dmnGraph_s <- delete.vertices(dmnGraph, degree(dmnGraph)==0) #delete.vertices(valGraph, degree(valGraph) < thresh2)
  
  # Data frame with coordinates and stuff
  metaStuff <- data.frame(Label = parcelCommunities[[1]]$Label,
                          x = parcelCommunities[[1]]$x,
                          y = parcelCommunities[[1]]$y,
                          z = parcelCommunities[[1]]$z,
                          Hmisphere = parcelCommunities[[1]]$Hemisphere,
                          valStr = strength(valGraph),
                          valBtw = betweenness(valGraph),
                          dmnStr = strength(dmnGraph),
                          dmnBtw = betweenness(dmnGraph))
  
  # Areas with strength above 95th percentile 
  # These will guide the following steps
  bootVal <- bootstrap(group = metaStuff$valStr)
  tempPr <- quantile(bootVal, probs=.95) # one sided test, since I'm interested in the highest-centrality nodes #quantile(metaStuff$valStr, probs=0.95)
  metaStuff$valFinal <- ifelse(metaStuff$valStr > tempPr, TRUE, FALSE)
  bootDMN <- bootstrap(group = metaStuff$dmnStr)
  tempPr <- quantile(bootDMN, probs=.95) # quantile(metaStuff$dmnStr, probs=0.95)
  metaStuff$dmnFinal <- ifelse(metaStuff$dmnStr > tempPr, TRUE, FALSE)
  metaStuff$bothFinal <- ifelse(metaStuff$valFinal & metaStuff$dmnFinal, TRUE, FALSE)# what areas are common between conditions
  
  # Get the labels that survive thresholding from the metanalysis
  dmnLbls <- as.character(metaStuff$Label[metaStuff$dmnFinal])
  dmnLbls <- gsub("L_","_",dmnLbls)
  dmnLbls <- gsub("R_","_",dmnLbls)
  dmnLbls <- unique(dmnLbls)
  valLbls <- as.character(metaStuff$Label[metaStuff$valFinal])
  valLbls <- gsub("L_","_",valLbls)
  valLbls <- gsub("R_","_",valLbls)
  valLbls <- unique(valLbls)
  bothLbls <- as.character(metaStuff$Label[metaStuff$bothFinal]) # Intersection of DMN/Val
  bothLbls <- gsub("L_","_",bothLbls)
  bothLbls <- gsub("R_","_",bothLbls)
  bothLbls <- unique(bothLbls)
  bothLbls <- bothLbls[-grep('V', bothLbls)] # remove visual areas
  combinedLbls <- unique(c(dmnLbls,valLbls)) # Combined DMN/Val (REMOVE V2 AND VLPFC!)
  combinedLbls <- gsub("L_","_",combinedLbls)
  combinedLbls <- gsub("R_","_",combinedLbls)
  combinedLbls <- unique(combinedLbls)
  combinedLbls <- combinedLbls[2:length(combinedLbls)]
  
  # And shorten the metaStuff dataframe (also useful later on for communities)
  metaStuff_dmn <- getCoords(Labels = dmnLbls, Coords = metaStuff)
  metaStuff_val <- getCoords(Labels = valLbls, Coords = metaStuff)
  metaStuff_both <- getCoords(Labels = bothLbls, Coords = metaStuff)
  metaStuff_comb <- getCoords(Labels = combinedLbls, Coords = metaStuff)
  
  # Strongest parcels per condition
  # As expected, 7m for DMN and a24 for DEC.
  # The argument can be that, while I'll plot the communities for all DMN areas, 
  # I'll focus on these ROIs in particular as they were also supported by the permutation test
...
if (Meta == T) {
  ...
  write("Setting up plots...", stdout())
  
  # Distributions
  test <- data.frame(dmnStrx = density(metaStuff$dmnStr)$x,
                     dmnStry = density(metaStuff$dmnStr)$y,
                     valStrx = density(metaStuff$valStr)$x,
                     valStry = density(metaStuff$valStr)$y,
                     dmnBtwx = density(metaStuff$dmnBtw)$x,
                     dmnBtwy = density(metaStuff$dmnBtw)$y,
                     valBtwx = density(metaStuff$valBtw)$x,
                     valBtwy = density(metaStuff$valBtw)$y)
  
  a1 <- ggplot(test) +
    geom_line(aes(x=dmnStrx, y=dmnStry, color = "DMN"), show.legend=FALSE) +
    geom_line(aes(x=valStrx, y=valStry, color = "Valuation"), show.legend=FALSE) +
    labs(x = "Strength", y = "Density") +
    theme_classic()
  a2 <- ggplot(test) +
    geom_line(aes(x=dmnBtwx, y=dmnBtwy, color = "DMN")) +
    geom_line(aes(x=valBtwx, y=valBtwy, color = "Valuation")) +
    labs(x = "Betweenness", y = "Density") +
    theme(axis.title.y = element_blank()) +
    scale_color_discrete(name="Condition") +
    theme_classic()
  #grid.arrange(h1,h2,ncol=2)
  
  # Plot the strength and betweenness onto the brain (metanalysis)
  b1 <- ggplot(metaStuff) +
    geom_nodes(aes(x=y, y=z, color = valStr,  size=valBtw), show.legend=FALSE) +
    geom_point(data = labelCoords_vertex, aes(x=y, y=z), alpha=0.01) +
    #scale_color_continuous(high = Cols[2]) +
    theme_blank()
  b2 <- ggplot(metaStuff) +
    geom_nodes(aes(x=x, y=y, color = valStr,  size=valBtw)) +
    scale_color_continuous(name="Strength") +
    scale_size_continuous(name="Betweenness") +
    geom_point(data = labelCoords_vertex, aes(x=x, y=y), alpha=0.01) +
    theme_blank()
  b3 <- ggplot(metaStuff) +
    geom_nodes(aes(x=y, y=z, color = dmnStr,  size=dmnBtw), show.legend=FALSE) +
    geom_point(data = labelCoords_vertex, aes(x=y, y=z), alpha=0.01) +
    theme_blank()
  b4 <- ggplot(metaStuff) +
    geom_nodes(aes(x=x, y=y, color = dmnStr,  size=dmnBtw)) +
    geom_point(data = labelCoords_vertex, aes(x=x, y=y), alpha=0.01) +
    scale_color_continuous(name="Strength") +
    scale_size_continuous(name="Betweenness") +  
    theme_blank()
  grid.arrange(b1,b2,b3,b4,a1,a2,ncol=2)
...
if (Parcellated == T) {
  ...
  # Modularity
  parcelCommunities <- mclapply(ptSeries, communityDetection, ROIS = c("_7m_", bothLbls), Type = "parcels", thresh = T)
...
if (Vertex) {
  ...
  # Modularity
  dmnval7mCommunities <- communityDetection(Data = timeSeries, ROIS = c("_7m_", bothLbls), Type = "vertex", thresh = T)
...
if (Vertex) {
  ...
  timeSeries_SNR_ROI <- lapply(timeSeries_SNR, getCoords, Labels = c("_7m_", bothLbls), TimeSeries = T)
  rm(timeSeries_SNR)
...
if (Vertex) {
  ...
  dmnval7mCommunities_halves <- lapply(timeSeries_halves, communityDetection, ROIS = c("_7m_", bothLbls), Type = "vertex", thresh = T)
...
if (Vertex == T) {
  ...
  # Modularity
  dmnval7mCommunities_sess <- lapply(timeSeries_sess, communityDetection, ROIS = c("_7m_", bothLbls), Type = "vertex", thresh = T)
...
if (Vertex & Sliding) {
  ...
  # Prep data
  ROI_timeSeries <- getCoords(Coords = timeSeries, Labels = c("_7m_", bothLbls), TimeSeries = T)
  
  # Sliding window analysis for each subject (using defaults)
  slideCommunities <- slidingWindow(subjTS = ROI_timeSeries, ROIs = c("_7m_", bothLbls), Spectral = T, Modularity = F, mins = 20, jump = 1)
