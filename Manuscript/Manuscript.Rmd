...
With these lists, we computed the number of times every two parcels co-occurred across each literature. This allowed us to generate two weighted networks in which each vertex was a brain parcel, and edges corresponded to the number of times each pair of them co-occurred across studies. Based on this, only edges with co-occurrences higher than 8 were preserved, and the remaining unconnected vertices (brain regions) were discarded. The connectivity strength of every pairwise parcels was then calculated as the sum of their co-occurrences. In order to threshold the parcels that are most likely to belong to each literature, we bootstrapped the connectivity strength and removed edges whose values lied below the 95th percentile of this distribution. In order to select regions of interest (ROI) from mPFC and other brain areas that are central to valuation and DMN literatures, we first bootstrapped the mean strength for each graph separately, and chose areas with strength above the 95th percentile of this distribution. Next, we took the intersection of these areas, such that the final ROIs were brain regions that overlapped between DMN and valuation in the literature (and thus our main target for dissociation).

Next, we identified areas that were solely related to either literature. We limited the search space to the parcels from Glasser et al. (2013) that were contained within the DMN parcellation defined by Yeo et al. (2011). We permuted the label of every study (i.e. DMN or valuation) 5000 times while preseving the total number of studies observed in each domain, and on each iteration stored the maximum statistic from a ROI-wise chi-squared test of proportions on the number of prmuted studies that had reported each ROI. This gave us a null distribution of chi-squared values that was used to determine which ROIs were more significantly prevalent on each literature. 

**fMRI Data**
In order to quantify the intrinsic connectivity of these ROIs in the brain, we acquired resting-state data from the Human Connectome Project (n = 80, randomly sampled from the total pool) Q6 release. Beyond the standard minimally preprocessed pipeline from HCP (), the data went through a number of additional refinements that are detailed elsewhere (Tobyne et al., 2017). Briefly, each dataset was bandpass filtered (DEETS HERE), and had the mean greyordinate signal regressed from every vertex time series (MGSR). Each subject's brain was comprised of 32k vertices per hemisphere. We retained only the cortical surfaces, which resulted in 59,412 total surface vertices per subject. Resting state data was acquired over four scanning sessions that occurred over the course of two days (two sessions per day). Each resting state scan was comprised of 1200 TRs (TR = 0.720s) for a total time of 14 min 33 s per session. We used this scanning structure to estimate each subject's topology based on session, day, and overall data (4800 TRs). 

**Within-ROI Connectivity**
To establish each subject's network, we selected all the vertices contained within the ROIs (n = 5,942 per subject) and computed the Pearson correlation of the time series of every pair of vertices. This produced a weighted network for each subject, where surface vertices were the nodes, and edges the correlations among all of them. Next, we took the exponential of the correlations, Fisher transformed them (i.e. tanh) so that all weights were positive while maintaining the shape of the original correlation distribution, and retained the edges whose adjusted p-values remained significant after correction for multiple tests (FDR < 0.05). 
