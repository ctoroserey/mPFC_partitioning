...
(Current: 880 words. Aim for 650)
  ...
  In this study, we aim to subsection regions commonly attributed to both DMN and subjective value into subject-specific DMN and non-DMN partitions, quantifying the degree of topographical heterogeneity present in this network within and across individuals. This description will allow researchers to generate more precise topographic targets for future studies of decision making. We do this by capitalizing on the respective strengths of meta-analytic and subject-specific analyses of brain data. First, we define regions of interest (ROI) by identifying overlapping activation throughout the literature projected to an established brain atlas (Glasser et al., 2013). We then produce a rsFC networks of all the surface vertices within these ROIs for each individual resting state dataset from the Human Connectome Project, and estimate the extent of DMN coverage through community detection. The resulting partitions are then evaluated for consistency across methods, throughout each individual's time series, and across subjects.

## Notes

I'll probably have to fix this in the intro: ADV et al note that dmPFC and vmPFC are more coactivated with DMN's PCC than pregenual ACC. This forms the topological shape that we report here. On my second read, it sounds like my interpretation is still correct: sure, vmPFC coactivates with subcortical value-related areas more than other mPFC sections, but this area also strongly coactivates with DMN in a way that they don't dissect. That's why they talk about the multifaceted embedding of psychological constructs here, even if the subregions they define are part of distinct networks.

It would be interesting to quantify the average percentage of the Yeo et al DMN netowrk that is utilized by each individual's DMN. That should be easy: mean(do.call(mean, apply(Summary, "[[", "FiedlerBinary"))). Also the average size: mean(do.call(sum, apply(Summary, "[[", "FiedlerBinary"))).

Gratton et al. (2018) have figures similar to ours indicating within subject stability being higher than across-subject similarity (3A). Our twist would be that we are quantifying a finer resolution level restricted to the ROIs (they use a 333 region parcellation), and they estimated it for the MSC only.9 So: what they find persists when examined at a finer level of resolution for a widespread sample.
...
**Meta-analysis**
We used data from metan-alyses that gathered peak brain coordinates of activity from studies of valuation (Bartra et al., 2010) (n = 27 studies) and DMN (Laird et al., 2009) (n = 77). These represent the surviving areas post-statistical thresholding from each study. For each peak volumetric coordinate, 10mm sphere masks were generated and then projected to a standard cortical mesh (fs_average, 32,000 vertices) using FreeSurfer. The sphere sections that overlapped with the cortex were mapped to an atlas of the human cortical surface (Glasser et al., 2016). This produced a list of standardized parcels that were reported on each study. 

With these lists, we computed the number of times every two parcels co-occurred across each literature. This allowed us to generate two weighted networks in which each vertex was a brain parcel, and edges correspond to the number of times each pair of them co-occurred across studies. In order to threshold the parcels that are most likely to belong to each lliterature, we bootstrapped the co-occurrences across studies and removed edges whose co-occurrences lied below the 90th percentile of this distribution. Based on this, only edges with co-occurrences higher than 8 were preserved, and the remaining unconnected vertices (brain regions) were discarded. The connectivity strength of every pairwise parcels was then calculated as the sum of their co-occurrences. In order to select regions of interest (ROI) from mPFC and other brain areas that are central to valuation and DMN literatures, we first bootstrapped the mean strength for each graph separately, and chose areas with strength above the 95th percentile of this distribution. Next, we took the intersection of these areas, such that the final ROIs were brain regions that overlapped between DMN and valuation in the literature (and thus our main target for dissociation).

**Whole Brain Parcellated connectivity**
In order to actually quantify the intrinsic connectivity of these ROIs in the brain, we acquired resting-state data from the Human Connectome Project (n = 80). Beyond the standard minimally preprocessed pipeline from HCP (), the data went through a number of additional refinements that are explained in detail elsewhere (Tobyne et al., 2017). Briefly, each dataset was bandpass filtered (DEETS HERE), and had the mean greyordinate signal regressed from every vertex time series (MGSR). Each data set contained around 1 hour of signals represented as approximately 4800 time points, and each subject contained 60,000 vertices. I parcellated each subject’s brain according to the atlas mentioned above, so that each parcel contained the mean time series from its vertices, and correlated the mean time series among all parcels (Pearson). This produced a weighted adjacency matrix for each subject, where each of the 360 parcels is a vertex, and edges the correlations among all of them. Next, I took the exponential of the correlations, Fisher transformed them (i.e. tanh) so that all weights were positive while maintaining the shape of the original correlation distribution, and retained the edges whose adjusted p-values remained significant (FDR < 0.05). Again, I quantified each parcel’s strength and betweenness, averaged them across subjects, and examined whether any of the ROIs showed uniquely high centralities. For this, I performed a permutation comparing the mean strength between the ROIs and the rest of the brain for each participant.
